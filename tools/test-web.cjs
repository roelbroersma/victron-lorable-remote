...
const server=http.createServer((req,res)=>{
 ...
 if(req.url==='/session')res.end(JSON.stringify({token:'test-token',native:true,esp_firmware:'4.11.0',wifi_clients:1,wifi_rssi_dbm:[-42],ble_target_mac:config.victron_mac,ble_target_state:1,ble_target_age_s:5}));
 else if(req.url==='/config')res.end(JSON.stringify(config));
 else if(req.url==='/status')res.end(JSON.stringify({...config,firmware:'4.11.0',uptime_s:4567,tx_count:3}));
...
(async()=>{
 ...
 assert.equal(await page.locator('#firmware_badge').textContent(),'LoRaBLE Remote · v4.11.0');
