...
const stm=path.join(root,'build_public411'),esp=path.join(root,'esp8684/build_release411');
...
const image=fs.readFileSync(path.join(root,'dist/release-4.11.0/LoRaBLE-Remote-4.11.0.bin'));
...
const localFile=path.join(root,'stm32/settings.local.h');
...
for(const name of ['status.png','networks.png'])add('docs/images/'+name,path.join(root,'test-results',name));
add('firmware/LoRaBLE-Remote-'+version+'.bin',path.join(root,'dist/release-4.11.0/LoRaBLE-Remote-4.11.0.bin'));
